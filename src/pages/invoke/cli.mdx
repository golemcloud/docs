...
The `golem` command line interface exposes two invocation subcommands:

- `golem worker invoke` triggers a function invocation on a worker without waiting for its completion
- `golem worker invoke-and-await` invokes a function on a worker and awaits its result
...
- Passing a JSON array of **JSON values** only, without type information, to `--parameters`. The format for encoding WebAssembly values in JSON is [described on the JSON WAVE Mapping page](/json-wave-mapping). In this case `golem` assumes that the invocation targets the latest version of the selected component, queries it's metadata and uses the selected function's type information for interpreting the provided JSON values.
...
```shell copy
golem worker cancel-invocation --worker urn:worker:<component-id>/<worker-name> --idempotency-key <idempotency-key>
```
