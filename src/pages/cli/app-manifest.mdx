...
On the other hand, deploying will always deploy the whole application atomically, so it is ensured that all our agents
are up-to-date.
...
To access Golem's own hosted Golem Cloud (or other custom installations), we can simply use the same commands as before
