...
<Tabs items={["TypeScript", "Rust", "Scala", "MoonBit"]} storageKey="nextra-tabs-Rust|TypeScript|MoonBit">
    ...
    <Tabs.Tab>
        ...
        | MoonBit type | WIT type | Example Rib value | Remarks
        | ------------ | -------- | ----------------- | -------
        | `String` | `string` | `"hello world"` | |
        | `Bool` | `bool` | `true`, `false` | |
        | `Double` | `f64` | `1234.0` | |
        | `Float` | `f32` | `3.14` | |
        | `Int` | `s32` | `-2147483648, 0, 2147483647` | |
        | `UInt` | `u32` | `0, 4294967295` | |
        | `Int64` | `s64` | `-9223372036854775808, 0, 9223372036854775807` | |
        | `UInt64` | `u64` | `0, 18446744073709551615` | |
        | `Byte` | `u8` | `0, 255` | |
        | `Bytes` | `list<u8>` | `[104, 101, 108, 108, 111]` | |
        | `Array[T]` | `list<T>` | `["one", "two", "three"]` | |
        | `Array[(K, V)]` | `list<tuple<K, V>>` | `[("key1", 100), ("key2", -2)]` | Use an array of tuples for key-value pairs |
        | `T?` | `option<T>` | `some(value)`, `none` | |
        | `#derive.golem_schema struct MyStruct { field1 : T1, field2 : T2 }` | `record { ... }` | `{ field1: "value", field2: 42 }` | |
        | `#derive.golem_schema enum MyEnum { Low, High }` | `enum { ... }` | `High` | All-unit enums map to WIT enums |
        | `#derive.golem_schema enum MyEnum { Coordinates(x~ : Double, y~ : Double), Empty }` | `variant { ... }` | `Coordinates({ x: 12.5, y: 7.0 })` | Payload enums map to WIT variants |
        | `Unit` | `unit` | `()` | |
        | `(T1, T2, T3)` | `tuple<...>` | `("hello", 1234, true)` | |
    </Tabs.Tab>
</Tabs>
