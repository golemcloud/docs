...
```shell copy
$ golem list-examples --language js
```
...
```shell copy
$ golem new --language js --package-name 'golem:demo' js-example
```
...
<Callout type="info">
During [componentization](/experimental-languages/js-language-guide/golem-js-sdk) the exported objects and functions are checked against the `wit` world, eg. if the helloWorld function were not exported, we would get the following error:
...
<Callout type="info">
    During [componentization](/experimental-languages/js-language-guide/golem-js-sdk) the exported objects and functions are checked against the `wit` world, eg. if the get function were not part of the `api` objects, we would get an error similar to this:
