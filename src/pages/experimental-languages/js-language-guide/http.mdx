...
HTTP request can be made with the JavaScript standard builtin `fetch` function, but handling the asynchronous return values inside the synchronous component functions requires helpers from the [Golem JavaScript SDK](/experimental-languages/ts-language-guide/golem-ts-sdk).
