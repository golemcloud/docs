...
```shell copy
golem list-examples --language ts
```
...
```shell copy
golem new --language ts --package-name 'golem:demo' ts-example
```
...
In addition to these, when using [Worker to Worker communication](/experimental-languages/ts-language-guide/rpc), workers created by remote calls **inherit the environment variables of the caller**.
