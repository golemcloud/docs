...
Make sure that all required tools are installed according to the [setup](/experimental-languages/ts-language-guide/setup) page, including the _Worker to Worker communication_ parts.
...
```shell copy
golem new --example ts-multi-rpc --package-name golem:demo ts-example
```
...
First create two TypeScript components using the techniques described in the [defining components](/experimental-languages/ts-language-guide/defining-components) page. The two components should be in two separate directories, enclosed by a root directory for the whole project.
...
```shell
golem new --example ts-default --package-name rpcdemo:component2 component2
golem new --example ts-default --package-name rpcdemo:component1 component1
```
...
A common pattern is to use environment variables to configure the deployed component's **component ID** and use that to construct the URI. See the [defining components](/experimental-languages/ts-language-guide/defining-components) page for more information about passing configuration values to workers.
...
```shell copy
golem stubgen compose --source-wasm out/component1.wasm --stub-wasm ../component2-stub/component2_stub.wasm --dest-wasm component1-with-component2-stub.wasm
```
