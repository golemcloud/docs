...
Store the required API key as a **secret** using Golem's typed config system. See the [`golem-add-secret-rust`](/how-to-guides/rust/golem-add-secret-rust) skill for full details. In brief:
...
- Provider API keys should be stored as secrets using Golem's typed config system (See the [`golem-add-secret-rust`](/how-to-guides/rust/golem-add-secret-rust) guide)
