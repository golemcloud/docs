...
Although Golem components are just regular WASM components, we recommend to use the `golem` command line interface to create new projects. It provides a set of predefined, Golem-specific **templates** to choose from as a starting point, and enables advanced features such as [worker to worker communication](/concepts/worker-to-worker-communication).

To see the available examples for Rust, run:

```shell copy
golem list-examples --language rust
```

Then to create a new project based on the default Rust example, run:

```shell
golem new --language rust --package-name 'golem:demo' example
```
