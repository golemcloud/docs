...
```
    ...
    <Tabs.Tab>
An agent is a Rust trait that is annotated with macro `#[agent_definition]`
...
The agent trait should have a construction method which returns `Self`, otherwise it will fail to compile
    </Tabs.Tab>
...
```
 ...
 Note that in order to use complex data types as parameters or return types of agent methods, these types need to implement the `Schema` trait (by deriving it using `#[derive(Schema)]`),
 so that the Golem runtime can serialize and deserialize them properly.

 Also note that you can also use `async` functions as agent methods in Rust. However, you don't need to use `async_trait` crate or similar, as the Golem SDK takes care of that for you.
