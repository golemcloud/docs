...
Golem is a serverless computing platform focused on high-reliability.

Through durable computing, workers executing on Golem are automatically fault-tolerant, update-tolerant, and flaky-tolerant.

Golem workers continue execution uninterrupted, Golem continue execution uninterrupted, even when hardware fails, servers go down, clusters are resized, or new updates are deployed.
...
- **Flaky-Tolerance**. While Golem is reliable, sometimes your system needs to talk to external systems. Golem performs sophisticated, user-configurable retry policies on all recoverable failures, freeing you from the drudgery of retries.
...
In the following examples, you will learn how several different types of applications could be architected for Golem.
...
10. Handle unexpected errors during fulfillment, or the user's cancellation of the order before dispatch.
...
With Golem, the entire order processing process can be written and deployed as a serverless function. When an API triggers an invocation of the function, the worker will run durably, regardless of faults, updates, or flakiness. This automatic reliability lets us focus on business logic, rather than the complexities of designing and maintaining an event-driven architecture.
...
1. **Financial Transaction Processing**. Taking bets and settlement, which involve traditional commerce processes such as the preceding storefront example.
2. **State Management**. Managing the state of the game session during play, as game play unfolds.
...
Each execution of a model takes time and resources, which equates to higher latency or higher costs. Due to these realities, it is often a technical requirement for AI agents to execute _durably_, such that they are fault-tolerant.
