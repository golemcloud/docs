...
To connect to a worker's log with `golem-cli`, use either the `golem-cli worker connect` command or the `--connect` flag when making an invocation. See the [CLI documentation](cli-workers) for more details.
