...
Using the WASI Logging interface depends on the guest language - for example for Rust, there is a crate providing bindings for the standard `log` crate as [described on the Building Components in Rust page](/docs/rust-language-guide/building-components.mdx).
...
To connect to a worker's log with `golem-cli`, use either the `golem-cli worker connect` command or the `--connect` flag when making an invocation. See the [CLI documentation](/docs/cli/workers.mdx) for more details.
