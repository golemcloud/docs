...
There are **precompiled binaries** of `golem-cli` (and its other variants) for various platforms. Alternatively you can build it for yourself.
...
The comman above installs both universal `golem-cli` (with support for cloud and open source version) and cloud specific `golem-cloud-cli`.

#### Alternative CLI versions

You can download the cloud specific `golem-cloud-cli` without supporting OSS deployments from the following page:
...
Alternatively you can build and install cloud specific `golem-cloud-cli` without universal `golem-cli` with the following command:

```bash filename="Terminal" copy
cargo install golem-cloud-cli
```

You can download the open source specific `golem-cli` without supporting Golem Cloud from the following page:

<Cards num={1}>
  <Card title="Golem Releases" href="https://github.com/golemcloud/golem/releases" />
</Cards>

Alternatively you can build and install open sorcce specific `golem-cli` without cloud profiles support with the following command:
...
We will use universal `golem-cli` in all examples, but `golem-cloud-cli` supports all commands supported by universal `golem-cli`.
...
See also [deployment](#deployment-overview) page for additional deployment options.
...
This command starts an iteractive configuration process. For non-interactive options - see [Golem CLI](/docs/golem-cli) page.
