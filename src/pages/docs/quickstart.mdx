...
You can install universal version of `golem-cli` with the following command:
...
You can install cloud specific `golem-cloud-cli` withour universal `golem-cli` with the following command:

```bash filename="Terminal" copy
cargo install golem-cloud-cli
```

You can install open sorcce specific `golem-cli` without cloud profiles support with the following command:
