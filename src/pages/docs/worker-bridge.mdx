  ...
  "routes": [
    {
      ...
      "binding": {
        ...
        "response": "${ { body: golem:it/api/get-cart-contents(), status: 200 } }"
      }
    }
  ]
}
...
**response**: The `response` field is an expression in which you  before sending it back to the client. For example, you can extract specific fields from the worker response, transform the data, or add additional information.
In fact you can write the `Rib` expression in this response field. Refer Rib documetation for more information

```rust

let worker_response = golem:it/api/get-cart-contents();

let extracted_data = match worker_response {
  ok(value) => value,
  err(msg) => msg
};

let status_code = match worker_response {
  ok(value) => 200,
  err(msg) => 500
};

{ body: extracted_data, status: status_code }

```

You can also choose to not do any transformation and simply call the worker function and leave it to the reasonable defaults.
Note that the last statement is the final outcome of the expression. In the above example, it is a wasm record in Rib Syntax (in-fact, it follows the WASM WAVE syntax)
...
In this case, whatever the worker response will be piped as it is as an http response body (defaulting to application/json content type) with a status code 200 in case of successful response from the worker,
...
```json
{
  "apiDefinitions": [{"id": "shopping-cart-v1", "version": "0.0.3"}],
  "site": "localhost:9006"
}
```
...
```bash filename="Terminal" copy
...
{ status: 200, body : golem:it/api/get-cart-contents(), headers: {etag: '1234567890abcdef'} }
```
...
"response" : "${ { status: 200, body : golem:it/api/get-cart-contents(), headers: {etag: '1234567890abcdef'} } }"
```
...
let worker_response = golem:it/api/get-cart-contents();
{
  status: 200,
  body : {
    name: worker_response[0].name,
    price: worker_response[0].price,
    quantity: worker_response[0].quantity
  }
  ...
  "routes": [
    {
      ...
      "binding": {
        ...
        "componentId": "2696abdc-df3a-4771-8215-d6af7aa4c408",
        "workerName": "worker-${request.path.user-id}",
        "response": "${ golem:it/api/get-cart-contents(request.path.user-id, 1, "hardcoded") }"
      }
    }
  ]
}
...
### Manipulating worker rspose

Rib expression language can be used to do any manipulations on top of the worker response as explained before.
More examples in this space can be found in the Rib expression language documentation.
