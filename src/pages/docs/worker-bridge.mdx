...
In the above example, we did not pass any parameters to the function. Let's say we need to pass the user-id to the function. In this case, the functionParams field will contain an array of expressions that evaluate to the parameters to be passed to the function.
  ...
  "routes": [
    {
      ...
      "binding": {
        ...
        "workerId": "worker-${request.path.user-id}",
        "functionName": "golem:it/api/get-cart-contents",
        "functionParams": [ "${request.path.user-id}" ],
        "response" : "${worker.response}"
      }
    }
  ]
