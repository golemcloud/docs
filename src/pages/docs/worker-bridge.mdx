...
The advantage of such an approach is reusability of the endpoint definition across your system. For example, you may want to use the same spec to upload to an external API gateway, which you can use for other features.
...
This will be part of each path object in the OpenAPI spec. In future we will support for x-golem-worker-bridge to be part of each method so that it supports
