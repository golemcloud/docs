...
Here is a simple API definition. You have to replace with the correct template-id and function name once you update your shopping-cart template to golem. Consider the above
as a simple example.
...
Sometimes, we represent the backend service using OpenAPI spec. Worker-bridge allows you to import OpenAPI spec and convert it to API definition.
Using extension feature of OpenAPI spec, you can inject the worker-bridge information into the OpenAPI spec. This is a powerful feature that allows you to define the API definition in OpenAPI spec and import it to worker-bridge.
The advantage of such an approach is re-usability of the endpoint definition across your system. For example, you may want to use the same spec to upload to an external API gateway, which you can use for other features.
