...
Once the `golem:api/host@0.2.0` interface is imported in the WIT file (see the [previous page for more information](docs/ccpp-language-guide/durability)), the retry policy can be controlled with the `golem_api_host_get_retry_policy` and `golem_api_host_set_retry_policy` functions.
...
The `golem_api_host_retry_policy_t` type itself is originatd from Golem's WIT definition in the following way:
