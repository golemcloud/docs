...
To create a promise simply call the `golem_api_host_golem_create_promise` function:

```c
golem_api_host_promise_id_t promise_id;
golem_api_host_golem_create_promise(&promise_id);
```
...
golem_api_host_promise_id_t promise_id;
// .. set up promise_id ..
golem_api_host_golem_delete_promise(&promise_id);
```
...
To await a promise, use the `golem_api_host_golem_await_promise` function:

```c
golem_api_host_promise_id_t promise_id;
// .. set up promise_id ..
example_list_u8_t payload; // will be filled by the call
golem_api_host_golem_await_promise(&promise_id, &payload);
```
...
To complete a promise from within a worker, use the `golem_api_host_golem_complete_promise` function:

```c
golem_api_host_promise_id_t promise_id;
example_list_u8_t payload;
// .. set up promise_id and payload ..
golem_api_host_golem_complete_promise(&promise_id, &payload);
```
