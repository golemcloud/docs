...
By default, all the above three steps are executed when using `golem-cli app build`, but they can be selected by using the
`--step` flag. This is intended to be used for more complex setups, when one would like to integrate `golem-cli app build`
