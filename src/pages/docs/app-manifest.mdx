...
<Fields>
  ...
  <Field meta={{
    ...
    ***Optional string enum of component types,*** accepted values:
    ...
    See [Ephemeral vs Durable Workers](/docs/concepts/workers#ephemeral-vs-durable-workers) for more information about
    component types.
...
></Field>
  ...
  <Field meta={{
    ...
    ***Optional map of profiles***, which can be used to provide multiple _build profiles_ for a component,
    ...
    When building, by default `golem-cli` will use `defaultProfile`, unless a specific profile is requested _and__
...
></Field>
    ...
    ***Required string enum***, accepted values:
    <EnumValues>
      <EnumValue meta={{value: "wasm-rpc"}}>
        ...
        With this dependency type the linking happens on servers side, so there is no need for generating and creating WAM RPC
        clients.
      </EnumValue>
  ...
  <Field meta={{
    ...
    ***Optional map of named templates***, which can be used in <FieldLink path="components.<component-name>.template" />.
    ...
    The `templates` field can be defined in multiple application manifest documents, but the _template names_ must be unique.
