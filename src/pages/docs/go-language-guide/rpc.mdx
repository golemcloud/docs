...
To actually be able to use the imported stub we regenerate our bindings, which in this example project can done with:
...
Now we can use `make build` to create our component with remote worker calls, which can be deployed to golem.
