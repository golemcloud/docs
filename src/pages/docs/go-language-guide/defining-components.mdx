...
<Callout type="warning">
  Currently TinyGo does not support the standard `os.Args` function, so we need to use the Golem GO
  SDK to get the command line arguments.
</Callout>
...
<Callout type="warning">
  Currently TinyGo does not support the standard `os` package's environment functions, so we need to
  use the Golem GO SDK to get the command line arguments.
</Callout>
