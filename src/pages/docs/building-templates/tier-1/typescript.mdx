...
```bash copy
jco transpile wit --no-namespaced-exports --stub --name main -o src && rm src/main.js && mv src/main.d.ts src/main.ts
```
...
They work the same as in [Javascript](/docs/building-templates/tier-1/javascript#using-additional-wit-files-optional). You can use them, but you'll have to provide the types yourself
