...
The [Worker Gateway](/docs/concepts/worker-gateway.mdx) service not only exposes the [low-level REST API for invocations](/docs/invoke/http.mdx) but also provides a way to define fully customizable HTTP APIs. Future versions will support other protocols as well such as gRPC or GraphQL.
...
See the [Rib reference](/docs/rib.mdx) for the full reference of the binding language.
...
As mentioned above, see the [Rib reference](/docs/rib.mdx) to know more about Rib language.
...
- Use the [Golem CLI](/docs/cli.mdx) tool
- Use the [Rest APIs](/docs/rest-api.mdx)
...
Response Mapping is under the `response` field under each binding. This is written using [Rib](/docs/rib.mdx) to call the worker function and manipulate it's results and return a value that's understandable for each protocol we use.
...
Here we manipulate the result from worker function using a match expression. More details on match expression is explained under [Rib](/docs/rib.mdx) documentation.
