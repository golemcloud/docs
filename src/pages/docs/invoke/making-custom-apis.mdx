...
Here is an example of an API definition, which we will use to expose `get-cart-contents` function (`golem:component/api.{get-cart-contents}` to be more specific) as an HTTP API endpoint.
...
with 1 more extra information which is `servers` block with the value of the URL of the Golem Worker Gateway, that tells the API gateway to route the request to worker-gateway.
Obviously, it depends on how you installed Tyk. If you installed Tyk in the same network as worker-gateway, you can use the localhost as servers. If you are using a separate docker network with Tyk,
