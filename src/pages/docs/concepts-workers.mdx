...
<Callout type="info">
  ...
  as well as dynamic state, which captures work-in-progress.
</Callout>
...
section on [invocation](#concepts-invocation).
...
Workers support the following operations:
 ...
 - **Cancellation**. Workers can be cancelled at any time, which causes the 
   worker to stop executing. Cancelled workers suffer from corrupted state, 
   arising from their abupt termination, and cannot be resumed or inspected.
