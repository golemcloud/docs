...
- `--filter "propery op value"` - filter examples: `--filter "name = worker-name"`, `--filter "version >= 0"`, `--filter "status = Running"`, `--filter "env.var1 = value"`. Can be used multiple times (AND condition is applied between them)
...
| golem-cli worker get --template-name &lt;text&gt; --worker-name &lt;text&gt;
| golem-cli worker list --template-name &lt;text&gt; [--filter &lt;filter&gt;] [--count &lt;count&gt;] [--cursor &lt;cursor&gt;] [--precise &lt;precise&gt;]
