...
```shell
$ golem-cli new --example python-minimal-actor --component-name py-example --package-name 'golem:demo'
```
...
The first time a component is compiled (see the [Building Components page](docs/python-language-guide/building-components) for details), a couple of Python files are get generated in the `bindings` directory. This module contains the base classes and imported functions and data types defined in the `WIT` file(s).
...
- import the module containing the base class to be implemented from the generated binding
  s
...
```python
# import the module
from bindings import py_example
...
```python
from bindings.py_example import exports
...
from bindings.py_example.exports import api
