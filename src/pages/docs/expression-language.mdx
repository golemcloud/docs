...
With that, let's explain the grammar of expression and a few examples and corner cases
...
A field can be selected from an `Expr` if the `Expr` is a `Record` type. For example `foo.user`
is a valid selection given `foo` is an expression variable that gets evaluated to a record.
...
An index can be selected from an `Expr` if the `Expr` is a `Sequence` type

```bash filename="Terminal" copy
[1, 2, 3][0]
```

A field can be selected from an `Expr` if the `Expr` is a `Sequence` type.
For example `foo[10]` is valid given `foo` is an expression variable that gets evaluated to a sequence.
...
`Option` in Expr is WASM Option, which can take the shape of `Some` or `None`.  This is similar to `Option` type in std `Rust`.
...
The syntax is more or less inspired from wasm wave.
...
of response from the `worker`, and therefore is visible.
