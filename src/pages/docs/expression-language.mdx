...
is a valid selection given `foo` is a expression variable that gets evaluated to a record.
...
For example `foo[10]` is valid given `foo` is a expression variable that gets evaluated to a sequence.
