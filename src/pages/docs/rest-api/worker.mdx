...
## Get metadata of a template workers

| Path                                  | Method | Protected |
| ------------------------------------- | ------ | --------- |
| `/v2/templates/{template_id}/workers` | GET    | Yes       |
...
| count   | integer | No       | Count of listed values, default: 50                                                                                                                                                |
| cursor  | integer | No       | Position where to start listing, if not provided, starts from the beginning. It is used to get the next page of results. To get next page, use the cursor returned in the response |
...
| Property  | Comparator             | Description                                                          |
| --------- | ---------------------- | -------------------------------------------------------------------- |
| name      | StringFilterComparator | Name of worker, example: `name = worker-name`                        |
| version   | FilterComparator       | Version of worker, example: `version >= 0`                           |
| status    | FilterComparator       | Status of worker, example: `status = Running`                        |
| env.[key] | StringFilterComparator | Environment variable of worker, example: `env.var1 = value`          |
| createdAt | FilterComparator       | Creation time of worker, example: `createdAt > 2024-04-01T12:10:00Z` |
...
POST version with more advance filtering support:

| Path                                       | Method | Protected |
| ------------------------------------------ | ------ | --------- |
| `/v2/templates/{template_id}/workers/find` | POST   | Yes       |
...
| --------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Name      | StringFilterComparator | Name of worker, example: `{ "type": "Name", "comparator": "Equal", "value": "worker-name" }`                                     |
| Version   | FilterComparator       | Version of worker, example: `{ "type": "Version", "comparator": "GreaterEqual", "value": 0 }`                                    |
| Status    | FilterComparator       | Status of worker, example: `{ "type": "Status", "comparator": "Equal", "value": "Running" }`                                     |
| Env       | StringFilterComparator | Environment variable of worker, example: `{ "type": "Env", "name": "var1", "comparator": "Equal", "value": "value" }`            |
| CreatedAt | FilterComparator       | Creation time of worker, example: `{ "type": "CreatedAt", "comparator": "Greater", "value": "2024-04-01T12:10:00Z" }`            |
| And       |                        | And filter combinator, example: `{ "type": "And", "filters": [ ... ] }`                                                          |
| Or        |                        | Or filter combinator, example: `{ "type": "Or", "filters": [ ... ] }`                                                            |
| Not       |                        | Creation time of worker, example: `{ "type": "Not", "filter": { "type": "Version", "comparator": "GreaterEqual", "value": 0 } }` |
