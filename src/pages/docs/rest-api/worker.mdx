...
| ------- | ------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| filter  | string  | No       | Filter for worker metadata in form of `property op value`. Filter examples: `name = worker-name`, `version >= 0`, `status = Running`, `env.var1 = value`. Can be used multiple times (AND condition is applied between them) |
