...
Currently, you cannot customize the retry strategy without forking the Golem 
code base and incorporating your modifications directly in the worker executor. 
However, configurable retry strategies are on the roadmap for future releases.
...
The following sections discuss these limitations in more depth and what you can
do to mitigate them.
...
performed by your worker are repeated.
...
may occur after the request has been sent, but before the response has been
received. In this scenario, during the recovery, the request must be retried, 
leading to the remote system receiving the request multiple times.
...
The availability of workers in a Golem cluster is affected by several different 
...
The primary way to improve availability in Golem is to increase the size of the 
Golem executor cluster. This will allow Golem to distribute workloads across
more nodes, reducing the likelihood of any single node becoming a bottleneck.
