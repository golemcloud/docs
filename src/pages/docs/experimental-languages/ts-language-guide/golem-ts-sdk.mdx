...
The Golem TypeScript SDK currently relies on forked versions of `jco` and `componentize-js`. Our examples contains the latest dependencies that work together, but in case you want setup you project manually, use the following sample `pacakge.json` as a starting point:
