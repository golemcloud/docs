...
First create two TypeScript components using the techniques described in the [defining components](/docs/experimental-languages/ts-language-guide/defining-components) page. The two components should be in two separate directories, enclosed by a root directory for the whole project.
...
To make the stub definition available for binding generation we have to import it in the callers world. To do se we have to
...
`component1-with-component2-stub.wasm` is now ready to be deploy to golem.
