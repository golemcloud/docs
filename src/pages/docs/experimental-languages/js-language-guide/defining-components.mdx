...
<Callout type="info">
During [componentization](/docs/experimental-languages/js-language-guide/golem-js-sdk) the exported objects and functions are checked against the `wit` world, eg. if the helloWorld function were not exported, we would get the following error:
...
<Callout type="info">
    During [componentization](/docs/experimental-languages/js-language-guide/golem-js-sdk) the exported objects and functions are checked against the `wit` world, eg. if the get function were not part of the `api` objects, we would get an error similar to this:
...
In addition to these, when using [Worker to Worker communication](/docs/pages/js-language-guide/worker-to-worker-communication), workers created by remote calls **inherit the environment variables of the caller**.
