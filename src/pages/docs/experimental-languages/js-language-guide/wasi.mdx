...
The [Golem JavaScript SDK](/docs/experimental-languages/js-language-guide/golem-js-sdk.mdx) provides idiomatic wrappers on a subset of these interfaces, but it is also possible to use the generated bindings directly.
...
```javascript
import { GetWorkers } from "golem:api/host@0.2.0"
...
```javascript
import { updateWorker } from "golem:api/host@0.2.0"
