...
Make sure that all required tools are installed according to the [setup](/ts-language-guide/setup) page, including the _Worker to Worker communication_ parts.
...
First create two TypeScript components using the techniques described in the [defining components](/ts-language-guide/defining-components) page. The two components should be in two separate directories, enclosed by a root directory for the whole project.
...
A common pattern is to use environment variables to configure the deployed component's **component ID** and use that to construct the URI. See the [defining components](/ts-language-guide/defining-components) page for more information about passing configuration values to workers.
