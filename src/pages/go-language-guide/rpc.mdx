...
Make sure that all required tools are installed according to the [setup](/go-language-guide/setup) page, including the _Worker to Worker communication_ parts.

## Setup with using example RPC Go project

The `golem new` command can be used to generate an example project which contains multiple components that are calling each other.
...
```shell
$ golem new --example go-multi-rpc --package-name golem:demo go-example
```
...
In case you prefer manually setting up your project, or if you want to learn more about how `golem stubgen` works, see the next section.

## Setup with directly using golem stubgen
...
First create two Go components using the techniques described in the [defining components](/go-language-guide/defining-components) page. The two components should be in two separate directories, enclosed by a root directory for the whole project.

<Callout type="warning">
    It is important to **use a different package name** for each component, otherwise the stub generator will fail.

    This can be done by using the `--package-name` parameter of `golem new` for example `--package-name rpcdemo:component1` and `--package-name rpcdemo:component2`.
...
In this document we will use the `golem new` command for creating the components:

```shell
golem new --example go-default --package-name rpcdemo:component1 component1
golem new --example go-default --package-name rpcdemo:component2 component2
```
...
Let's create one for component2 using `golem stubgen build`:

```shell
golem stubgen build --source-wit-root component2/wit --dest-wasm component2-stub/component2_stub.wasm --dest-wit-root component2-stub/wit
```
...
Now we add the generated stub definition to the caller project with the `golem stubgen add-stub-dependency`

```shell copy
golem stubgen add-stub-dependency --stub-wit-root component2-stub/wit  --dest-wit-root component1/wit --overwrite
```
...
A common pattern is to use environment variables to configure the deployed component's **component ID** and use that to construct the URI. See the [defining components](/go-language-guide/defining-components) page for more information about passing configuration values to workers.
...
For this we will add the following `golem stubgen compose` command as the last step in our makefile:

```shell copy
golem stubgen compose --source-wasm component1.wasm --stub-wasm ../component2-stub/component2_stub.wasm --dest-wasm component1-with-component2-stub.wasm
```
