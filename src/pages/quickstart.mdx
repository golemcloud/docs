...
<Tabs items={["TypeScript", "Rust"]}>
  ...
  > Create application or Add another component? Create application
  ```
  ...
  Note: For TypeScript, you need to have `npm` installed on your system.
    ...
    > Select a template for the new component: default: The default agent template for Rust
    > Component Package Name: example:counter
    > Create application or Add another component? Create application
    ```
    </Tabs.Tab>
...
<Tabs items={["TypeScript", "Rust"]}>
  <Tabs.Tab>
The default template's source code is located in the `components-ts/example-counter/src/main.ts` file, assuming that the `example:counter` name has been used in the `app new` command. The directory structure allows applications to have multiple components, even implemented in multiple programming languages if needed.
...
```
 ...
 <Tabs.Tab>
  The default template's source code is located in the `components-rust/example-counter/src/lib.rs` file, assuming that the `example:counter` name has been used in the `app new` command.
...
To finish the Quickstart example with an HTTP POST endpoint for tracking counts, let's open the `golem.yaml` file in the component's directory. It's going to contain a `components` and a `dependencies` section already.
