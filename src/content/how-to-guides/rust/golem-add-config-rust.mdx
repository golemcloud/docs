...
```shell
golem agent new 'MyAgent("instance-1")' \
  --config foo=42 \
  --config bar=hello \
...
- If the config includes `Secret<T>` fields, also use [`golem-add-secret-rust`](/how-to-guides/rust/golem-add-secret-rust) for secret-specific declaration and CLI guidance.
